...
class FactorVAEDataModule(L.LightningDataModule):
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        if self.use_synthetic:
            ...
            T = dc["sequence_length"]
            self._train = RealDataset(dc["processed_dir"], dc["train_start"], dc["train_end"], T)
            self._val   = RealDataset(dc["processed_dir"], dc["val_start"],   dc["val_end"],   T)
            self._test  = RealDataset(dc["processed_dir"], dc["test_start"],  dc["test_end"],  T)
